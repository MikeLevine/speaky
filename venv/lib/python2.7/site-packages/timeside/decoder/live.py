...
class LiveDecoder(FileDecoder):
   ...
   capturing audio from alsasrc
    ...
    def __init__(self, num_buffers=-1, input_src='alsasrc', vad=False):
    ...
    def setup(self, channels=None, samplerate=None, blocksize=None):
        ...
        if self.vad:
            vad_elem = '! vader name=vad auto-threshold=true'
        else:
            vad_elem = ''
        ...
        self.pipe = '''%s num-buffers=%d name=src
                       ...
                       ! audioresample ! audio/x-raw-float,channels=1,width=32,rate=8000 %s
                       ! appsink name=sink sync=False async=True
                       ''' % (self.input_src, self.num_buffers, vad_elem)
        ...
        self.conv = self.pipeline.get_by_name('convert')
        self.conv.get_pad("sink").connect("notify::caps", self._notify_caps_cb)
        if self.vad:
            vader = self.pipeline.get_by_name('vad')
            vader.connect('vader-start', self._vader_start_cb)
            vader.connect('vader-stop', self._vader_stop_cb)
