...
"""
    ...
    def run(self, channels=None, samplerate=None, blocksize=None):
        ...
        # Changed channels to always be mono
        source.setup(channels=1, samplerate=samplerate,
                     blocksize=blocksize)
#        print '\nsource [%s] is set up' % source.id()
        print 'Start recording your speech!\n'
...
#            print 'item [%s] is set up' % item.id()
            if item.id() == 'level':
                level = item
            elif item.id() == 'aubio_pitch':
                pitch = item
            elif item.id() == 'yaafe':
                yaaf = item
            elif item.id() == 'irit_speech_4hz':
                seg_syll = item
            elif item.id() == 'irit_speech_entropy':
                seg_entropy = item
            last = item
...
#            bufcount += 1
            ...
            ### HACK!!! ###
            if frames is None or numpy.all(numpy.isnan(frames)):
                continue
#            print '%d frames decoded' % len(frames)
            for item in items:
#                if item.id() != 'yaafe':
#                    print 'Processing with %s...' % str(item)
                frames, eod = item.process(frames, eod)
#                print '%d frames returned by %s' % (len(frames), str(item))
