...
#            import pdb; pdb.set_trace()
            stack = frames.copy()
            nb_frames = ((len(stack) - (self.blocksize // 2)) // self.stepsize) + 2
            nb_endpads = (self.stepsize * (nb_frames - 1)) + (self.blocksize // 2) - len(stack)
            
            if nb_endpads > 0:
                if len(frames.shape) == 1:
                    stack = np.concatenate([np.zeros(self.blocksize // 2, dtype=frames.dtype),
                                            stack, np.zeros(nb_endpads, dtype=frames.dtype)])
                else:
                    stack = np.concatenate([np.zeros((self.blocksize // 2, stack.shape[1]), dtype=frames.dtype),
                                            stack, np.zeros((nb_endpads, stack.shape[1]), dtype=frames.dtype)])
            
            # nb_frames = (stack_length - self.blocksize + self.stepsize) // self.stepsize
...
# #            self.buffer = stack[nb_frames * self.stepsize:]
            ...
            assert((len(stack) - self.blocksize) % self.stepsize == 0)
            eod_list = np.repeat(False, nb_frames)
            if eod and len(eod_list):
                eod_list[-1] = eod

            for i, eod in zip(xrange(0, nb_frames * self.stepsize, self.stepsize), eod_list):
                yield (stack[i:i + self.blocksize], eod)
