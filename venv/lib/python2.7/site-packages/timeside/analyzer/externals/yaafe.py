...
#################ADDDING CODE###################
        ...
        if self.input_samplerate:
            samprate = self.input_samplerate
        else:
            samprate = self.source_samplerate
        print 'Read %d lpc result frames' % len(lpc_frames)
        formants = [[] for frame in lpc_frames]
        for lpcs in lpc_frames:
            roots = [root for root in np.roots(lpcs) if root >= 0]
