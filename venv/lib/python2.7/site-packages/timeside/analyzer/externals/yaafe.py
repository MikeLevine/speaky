...
#        self.loud = None
    ...
    def check_loudness(self, loud_frames, energy_frames):
        for i in np.arange(self._LOUD_WINDOW, loud_frames.shape[0] + 1, self._LOUD_WINDOW):
            ...
            print 'loudvals:\t%.1f %.4f' % (loudness, energy)
#            loudness = (loudness + np.median(window)) / 2.0
            if loudness < 7 and self.quiet:
                print('\t^^^SPEAK UP!! (loudness)^^^')
...
#    def check_streak(cond, is_streak, thresh, warn_msg, fixed_msg=FIXED_MSG):
    ...
    def _find_streaks(self, indices):
        ...
        while i < len(indices) - FP_CONSEC_THRESH:
            ...
            if np.array_equal(np.ediff1d(indices[i:i+FP_CONSEC_THRESH]), np.ones(FP_CONSEC_THRESH-1)):
                msg = 'Filled Pause! found starting at frame %d\n' % indices[i]
                print msg
                self.log.write(msg)
                found = True
                self.fp_count += 1
...
#        import pdb; pdb.set_trace()
        f0fps = np.where(mx[:, 0] > .2)[0]
        f1fps = np.where(mx[:, 1] > .2)[0]
        f2fps, f3fps = np.nonzero(mx[:, 2] > .2)[0], np.nonzero(mx[:, 3] > .2)[0]
...
#            print 'Read %d lpc result frames' % len(lpc_frames)
            ...
            if np.any(loud_frames) and np.any(energy_frames):
                self.check_loudness(loud_frames, energy_frames)
