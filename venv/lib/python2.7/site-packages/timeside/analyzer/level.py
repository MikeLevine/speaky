...
class Level(Analyzer):
    ...
    def process(self, frames, eod=False):
        ...
        if frames.size:
            ...
            # rms_level
            mean = np.mean(np.square(frames))
            self.mean_values = np.append(self.mean_values, mean)
            rms = np.round(20 * np.log10(np.sqrt(mean)), 3)
            print 'level: %.2f dbfs' % rms
            if rms < -30.0 and self.times % 20 == 0:
#                print '\n^^^^^^^^Speak Up!^^^^^^^^'
                self.toosoft = True
            elif self.toosoft and rms > -30.0:
#                print '\n--------Much Better--------'
                self.toosoft = False
